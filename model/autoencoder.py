...
class AE(nn.Module):
    def __init__(self, in_feas_dim, latent_dim, a=0.5, b=0.5):
    ...
    def train_AE(self, train_loader, learning_rate=0.001, device=torch.device('cpu'), epochs=150, dataset="TCGA"):
        ...
        for epoch in range(epochs):
            ...
            for (x, y) in train_loader:
                ...
                loss = self.a * loss_function(decoded_omics_1, omics_1) + self.b * loss_function(decoded_omics_2, omics_2)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        ...
        plt.xlabel('epochs')
        plt.ylabel('loss')
        plt.savefig('result/{}/AE/AE_train_loss.png'.format(dataset))
